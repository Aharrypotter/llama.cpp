...
static void ggml_compute_forward_gelu_erf_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        const float * x = (float *) ((char *) src0->data + i1*(src0->nb[1]));
        float * y = (float *) ((char *) dst->data  + i1*( dst->nb[1]));

        for (int j = 0; j < nc; j++) {
            const float v = x[j];
            y[j] = 0.5f * v * (1.0f + erff(v * inv_sqrt_2));
        }
    }
}
...
static void ggml_compute_forward_expm1_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_expm1(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_expm1_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_floor_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_floor(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_floor_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_ceil_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_ceil(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_ceil_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_trunc_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_trunc(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_trunc_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_xielu_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < nc; j++) {
            const float v = x[j];
            if (v > 0.0f) {
                y[j] = alpha_p * v * v + beta * v;
            } else {
                const float min_v_eps = fminf(v, eps);
                y[j] = (expm1f(min_v_eps) - v) * alpha_n + beta * v;
            }
        }
    }
}

static void ggml_compute_forward_xielu(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_xielu_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
void ggml_compute_forward_soft_max_back(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
void ggml_compute_forward_unary(
    ...
    switch (op) {
        ...
        default:
            {
                GGML_LOG_ERROR("HTP: unsupported unary op=%d (%s) tensor=%s\n",
                               op, ggml_unary_op_name(op), dst->name);
                GGML_ABORT("fatal error");
            }
    }
}
...
static void ggml_compute_forward_reglu_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = src0_p[k];
            dst_p[k] = x > 0.0f ? x * src1_p[k] : 0.0f;
        }
...
static void ggml_compute_forward_reglu_f16(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = GGML_FP16_TO_FP32(src0_p[k]);
            const float g = GGML_FP16_TO_FP32(src1_p[k]);
            dst_p[k] = GGML_FP32_TO_FP16(x > 0.0f ? x * g : 0.0f);
        }
...
static void ggml_compute_forward_geglu_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            dst_p[k] = ggml_gelu_f32(src0_p[k]) * src1_p[k];
        }
...
static void ggml_compute_forward_geglu_f16(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = GGML_FP16_TO_FP32(src0_p[k]);
            const float g = GGML_FP16_TO_FP32(src1_p[k]);
            dst_p[k] = GGML_FP32_TO_FP16(ggml_gelu_f32(x) * g);
        }
...
static void ggml_compute_forward_swiglu_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            dst_p[k] = ggml_silu_f32(src0_p[k]) * src1_p[k];
        }
...
static void ggml_compute_forward_swiglu_f16(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = GGML_FP16_TO_FP32(src0_p[k]);
            const float g = GGML_FP16_TO_FP32(src1_p[k]);
            dst_p[k] = GGML_FP32_TO_FP16(ggml_silu_f32(x) * g);
        }
...
static void ggml_compute_forward_geglu_erf_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = src0_p[k];
            dst_p[k] = 0.5f * x * (1.0f + erff(x * SQRT_2_INV)) * src1_p[k];
        }
...
static void ggml_compute_forward_geglu_erf_f16(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = GGML_FP16_TO_FP32(src0_p[k]);
            const float g = GGML_FP16_TO_FP32(src1_p[k]);
            dst_p[k] = GGML_FP32_TO_FP16(0.5f * x * (1.0f + erff(x * SQRT_2_INV)) * g);
        }
...
static void ggml_compute_forward_geglu_quick_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            dst_p[k] = ggml_gelu_quick_f32(src0_p[k]) * src1_p[k];
        }
...
static void ggml_compute_forward_geglu_quick_f16(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int k = 0; k < nc; ++k) {
            const float x = GGML_FP16_TO_FP32(src0_p[k]);
            const float g = GGML_FP16_TO_FP32(src1_p[k]);
            dst_p[k] = GGML_FP32_TO_FP16(ggml_gelu_quick_f32(x) * g);
        }
...
void ggml_compute_forward_glu(
    ...
    switch (op) {
        ...
        default:
            {
                GGML_LOG_ERROR("HTP: unsupported glu op=%d (%s) tensor=%s\n",
                               op, ggml_glu_op_name(op), dst->name);
                GGML_ABORT("fatal error");
            }
    }
}
...
static void ggml_compute_forward_gated_delta_net_one_chunk(
    ...
    struct ggml_tensor * src_q     = dst->src[0];
    struct ggml_tensor * src_k     = dst->src[1];
    struct ggml_tensor * src_v     = dst->src[2];
    struct ggml_tensor * src_g     = dst->src[3];
    struct ggml_tensor * src_beta  = dst->src[4];
    struct ggml_tensor * src_state = dst->src[5];
...
static void ggml_compute_forward_gated_delta_net_f32(
    ...
    // 4x chunks per thread for better load balancing
    ...
    // Initialize chunk counter (skip first nth chunks already assigned)
    if (ith == 0) {
        atomic_store_explicit(&params->threadpool->current_chunk, nth, memory_order_relaxed);
    }
...
// NOTE(hzx): The following map_unary/binary/custom_f32 functions were removed
// because the old GGML API (ggml_unary_op_f32_t, ggml_binary_op_f32_t, ggml_custom*_op_f32_t)
// is no longer available in the current GGML version.
// The new API uses ggml_custom*_op_t with different signatures.
...
void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        // NOTE(hzx): GGML_OP_MAP_UNARY, MAP_BINARY, MAP_CUSTOM*_F32 removed
        // These old GGML ops are no longer supported in current GGML version.
        // Use GGML_OP_UNARY and GGML_OP_MAP_CUSTOM* instead.
